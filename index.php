<?php
session_start();

// Redirige si el usuario ya está logueado
if (isset($_SESSION['user_id'])) {
    header('Location: dashboard.php');
    exit;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Muebles - Inicio</title>
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700&family=Raleway:wght@400;700&display=swap" rel="stylesheet">
    <style>
        :root {
            --color-primario: #2c3e50;
            --color-secundario: #27ae60;
            --color-terciario: #3498db;
            --texto-claro: #ffffff;
            --texto-oscuro: #333333;
            --gris-claro: #ecf0f1;
            --sombra: 0 4px 6px rgba(0, 0, 0, 0.1);
        }

        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: 'Raleway', sans-serif;
            line-height: 1.6;
            color: var(--texto-oscuro);
            background-color: #f9f9f9;
        }

        .container {
            width: 90%;
            max-width: 1200px;
            margin: 0 auto;
            padding: 0 15px;
        }

        /* Header */
        header.header {
            background-color: var(--color-primario);
            color: var(--texto-claro);
            padding: 20px 0;
            position: relative;
        }

        header.header .container {
            display: flex;
            justify-content: space-between;
            align-items: center;
        }

        .logo {
            font-family: 'Playfair Display', serif;
            font-size: 28px;
            font-weight: 700;
            color: var(--texto-claro);
            text-decoration: none;
        }

        .logo p {
            font-family: 'Raleway', sans-serif;
            font-size: 14px;
            margin-top: 5px;
            font-weight: 400;
        }

        /* Botones de autenticación */
        .auth-buttons {
            display: flex;
            gap: 15px;
        }

        .auth-btn {
            padding: 8px 20px;
            border-radius: 4px;
            text-decoration: none;
            font-weight: bold;
            transition: all 0.3s;
            font-size: 14px;
        }

        .btn-login {
            background-color: transparent;
            color: var(--texto-claro);
            border: 2px solid var(--texto-claro);
        }

        .btn-login:hover {
            background-color: rgba(255, 255, 255, 0.1);
        }

        .btn-register {
            background-color: var(--color-secundario);
            color: var(--texto-claro);
            border: 2px solid var(--color-secundario);
        }

        .btn-register:hover {
            background-color: #2ecc71;
            border-color: #2ecc71;
        }

        /* Hero Section */
        section.hero {
            background-color: var(--gris-claro);
            padding: 80px 0;
            text-align: center;
        }

        section.hero h2 {
            font-family: 'Playfair Display', serif;
            font-size: 2.2rem;
            color: var(--color-primario);
            margin-bottom: 20px;
        }

        section.hero p {
            font-size: 1.1rem;
            max-width: 700px;
            margin: 0 auto 30px;
            color: #555;
        }

        /* Botones */
        .btn {
            display: inline-block;
            padding: 12px 30px;
            background-color: var(--color-secundario);
            color: var(--texto-claro);
            text-decoration: none;
            border-radius: 4px;
            font-weight: bold;
            transition: background-color 0.3s;
            margin: 5px;
        }

        .btn:hover {
            background-color: #2ecc71;
        }

        .btn-secondary {
            background-color: transparent;
            color: var(--color-primario);
            border: 2px solid var(--color-primario);
        }

        .btn-secondary:hover {
            background-color: var(--color-primario);
            color: var(--texto-claro);
        }

        /* About Section */
        section.about {
            padding: 60px 0;
            background-color: var(--texto-claro);
        }

        section.about h2 {
            font-family: 'Playfair Display', serif;
            font-size: 2rem;
            color: var(--color-primario);
            margin-bottom: 20px;
            text-align: center;
        }

        section.about p {
            max-width: 800px;
            margin: 0 auto;
            text-align: center;
            font-size: 1.1rem;
            line-height: 1.8;
        }

        /* Productos Destacados */
        section.featured-products {
            padding: 60px 0;
            background-color: var(--gris-claro);
        }

        section.featured-products h2 {
            font-family: 'Playfair Display', serif;
            font-size: 2rem;
            color: var(--color-primario);
            margin-bottom: 40px;
            text-align: center;
        }

        .product-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
            gap: 30px;
        }

        .product-card {
            background-color: var(--texto-claro);
            border-radius: 8px;
            overflow: hidden;
            box-shadow: var(--sombra);
            transition: transform 0.3s;
        }

        .product-card:hover {
            transform: translateY(-5px);
        }

        .product-card img {
            width: 100%;
            height: 200px;
            object-fit: cover;
        }

        .product-card h3 {
            font-family: 'Playfair Display', serif;
            font-size: 1.4rem;
            padding: 15px 20px 5px;
            color: var(--color-primario);
        }

        .product-card p {
            padding: 0 20px 20px;
            color: #666;
        }

        /* Footer */
        footer.footer {
            background-color: var(--color-primario);
            color: var(--texto-claro);
            padding: 30px 0;
            text-align: center;
        }

        footer.footer p {
            font-size: 14px;
        }
    </style>
</head>
<body>
    <!-- Header con botones a la derecha -->
    <header class="header">
        <div class="container">
            <a href="index.php" class="logo">Muebles <p>Donde el diseño y la comodidad se encuentran</p></a>
            <div class="auth-buttons">
                <a href="login.php" class="auth-btn btn-login">Iniciar Sesión</a>
                <a href="register.php" class="auth-btn btn-register">Registrarse</a>
            </div>
        </div>
    </header>

    <!-- Hero Section -->
    <section class="hero">
        <div class="container">
            <h2>Bienvenido a nuestra tienda</h2>
            <p>Explora nuestra exclusiva colección de muebles para tu hogar.</p>
            <a href="public.php" class="btn">Ver Productos</a>
        </div>
    </section>

    <!-- About Section -->
    <section class="about">
        <div class="container">
            <h2>Sobre Nosotros</h2>
            <p>En <strong>Muebles</strong>, nos dedicamos a ofrecerte los mejores muebles con diseños modernos y clásicos. Nuestra misión es transformar tus espacios en lugares únicos y acogedores.</p>
        </div>
    </section>

    <!-- Productos Destacados -->
    <section class="featured-products">
        <div class="container">
            <h2>Productos Destacados</h2>
            <div class="product-grid">
                <div class="product-card">
                    <img src="https://via.placeholder.com/300x200" alt="Sofá moderno">
                    <h3>Sofá Moderno</h3>
                    <p>Diseño elegante y cómodo para tu sala de estar.</p>
                </div>
                <div class="product-card">
                    <img src="https://via.placeholder.com/300x200" alt="Mesa de comedor">
                    <h3>Mesa de Comedor</h3>
                    <p>Perfecta para reuniones familiares y cenas especiales.</p>
                </div>
                <div class="product-card">
                    <img src="https://via.placeholder.com/300x200" alt="Cama king size">
                    <h3>Cama King Size</h3>
                    <p>Confort y estilo para tu dormitorio.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- Footer -->
    <footer class="footer">
        <div class="container">
            <p>&copy; <?php echo date('Y'); ?> Muebles. Todos los derechos reservados.</p>
        </div>
    </footer>
</body>
</html>